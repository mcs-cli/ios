...
The generated `.xcodebuildmcp/config.yaml` pins the default platform to `iOS`, isolates DerivedData at `./.xcodebuildmcp/DerivedData/` so the LSP always reads from a stable path, leaves `suppressWarnings` off, turns on test timing output, and enables these workflows:

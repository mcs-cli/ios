...
- **Navigation** (jump to a definition, follow one reference, look up a signature, list a file's symbols): use `hover`, `goToDefinition`, `workspaceSymbol`, `documentSymbol`. Enforced at tool-call time: whole-file `Read` on `.swift` and bare-identifier `Grep` with no scope are blocked (exit 2). Targeted reads (with `offset`/`limit`) and scoped Greps (glob, path, or word-boundary regex) pass through — those are audit shapes.
